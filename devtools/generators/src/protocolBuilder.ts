...
function emptySchemaNames(schemas: Schema[]): Set<string> {
  return new Set(
    schemas.filter((schema) => schema.fields.length === 0).map((schema) => schema.name)
  );
}
...
export function buildProtocolDefinition(source: ProtocolSourceModel): ProtocolModel {
  ...
  return loadProtocolDefinitionFromRaw(
    source.specRoot,
    path.join(source.specRoot, "protocol", "axtp.protocol.yaml"),
    raw
  );
}

export async function writeProtocolDefinition(
