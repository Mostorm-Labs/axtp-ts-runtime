...
function enumBlock(name: string, type: string, items: { name: string; id: number }[]): string {
  ...
  const rows = sortById(items)
    .map((item) => `    ${cppName(item.name)} = ${hex(item.id, width)},`)
    .join("\n");
...
function emitSchema(spec: SpecModel): string {
  const fieldArrays = spec.schemas
    .map((schema) => {
  ...
  const descriptors = spec.schemas
    .map(
      (schema) =>
...
function emitEventRegistry(spec: SpecModel): string {
  const rows = sortById(spec.events)
    .map(
      (item) => `    { ${hex(item.id)}, "${item.name}", "${item.domain}", "${item.eventSchema}" },`
    )
    .join("\n");
  return `${banner}#pragma once
#include <cstddef>
...
function emitErrorRegistry(spec: SpecModel): string {
  const rows = sortById(spec.errors)
    .map((item) => `    { ${hex(item.id)}, "${item.name}", "${item.domain}", ${item.retryable} },`)
    .join("\n");
  return `${banner}#pragma once
#include <cstddef>
#include <cstdint>
...
function emitTlvSkeleton(spec: SpecModel): string {
  ...
  const declarations = spec.schemas
    .map((schema) => {
...
function emitMethodTraits(spec: SpecModel): string {
  const methods = sortById(spec.methods)
    .map(
      (method) => `template <>
struct MethodTraits<MethodId::${cppName(method.name)}> {
  ...
  const events = sortById(spec.events)
    .map(
      (event) => `template <>
...
function emitFieldNamespace(schema: Schema): string {
  const rows = schema.fields
    .map((field) => `constexpr std::uint8_t ${cppConstName(field.name)} = ${hex(field.id, 2)};`)
    .join("\n");
