...
export async function emitTestVectorFiles(_spec: SpecModel, dir: string): Promise<void> {
  ...
  const vectors: Vector[] = [
    ...
    {
      name: "rpc_audio_get_algorithm_config_request",
      payloadType: "RPC",
      encoding: "tlv",
    ...
    {
      name: "rpc_audio_set_algorithm_config_request",
      payloadType: "RPC",
      encoding: "tlv",
    ...
    {
      name: "event_audio_algorithm_config_changed",
      payloadType: "RPC",
      encoding: "tlv",
      hexFile: "event_audio_algorithm_config_changed.hex",
      expectDecode: { event: "audio.algorithmConfigChanged", field: "noiseSuppression.level" }
    },
    {
    ...
    {
      name: "compact_crc8_error",
      payloadType: "RPC",
      encoding: "tlv",
      hexFile: "compact_crc8_error.hex",
      expectError: "FRAME_CRC_ERROR"
    },
    {
      name: "compact_message_id_overflow",
      payloadType: "RPC",
      encoding: "tlv",
      hexFile: "compact_message_id_overflow.hex",
      expectError: "COMPACT_MESSAGE_ID_OVERFLOW"
    }
  ];
  ...
  const hexData: Record<string, string> = {
    ...
    "event_audio_algorithm_config_changed.hex":
      "41580102000E0110000400010206000000000901000001010101",
  ...
  await Promise.all([
    ...
    ...Object.entries(hexData).map(([file, content]) =>
      writeTextFile(path.join(dir, file), content)
    )
  ]);
}
